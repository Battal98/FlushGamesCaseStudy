...
public static class ExtentionsHelper
{
    ...
    #region Save And Load Jobs

    public static void SaveDataToFile<T>(this T saveData, string folderName, string fileName)
    {
    ...
    private static string ConvertFilePathToJSON(string folderName, string fileName)
    {
        string folderPath = Path.Combine(Application.persistentDataPath, folderName);
        Directory.CreateDirectory(folderPath);
        string filePath = Path.Combine(folderPath, fileName + DATA_FORMAT);
        return filePath;
    }
